...
GMAIL_SENDER   = os.environ.get("GMAIL_SENDER", "martin.dailerian@gmail.com")
GMAIL_APP_PASS = os.environ.get("GMAIL_APP_PASS", "rmep fexi gqhv lopd")
SEND_TIME = "16:00"
...
def grade_badge(letter, score):
    ...
    return f'<span style="background:{bg};color:{fg};padding:3px 10px;border-radius:99px;font-family:monospace;font-size:13px;font-weight:600;">{letter} {s}</span>'
...
def build_rows(grades, assignments=None):
    ...
    for i, c in enumerate(grades):
        ...
        rows += f"<tr style='background:{bg};'><td style='padding:10px 14px;border-bottom:1px solid #eee;font-size:13px;'><strong>{c['subject']}</strong><br><span style='color:#888;font-size:11px;'>{c['teacher']}</span></td><td style='padding:10px 14px;border-bottom:1px solid #eee;text-align:center;'>{badge}</td><td style='padding:10px 14px;border-bottom:1px solid #eee;font-size:12px;line-height:1.6;'>{assign}</td></tr>"
    return rows
...
def build_email():
    ...
    if alerts:
        ...
        alert_html = f'<div style="background:#fff3cd;border-left:4px solid #ffc107;padding:14px 18px;margin-bottom:24px;border-radius:6px;"><strong style="color:#856404;">Alerts requiring attention</strong><ul style="margin:8px 0 0;padding-left:18px;color:#856404;font-size:13px;">{items}</ul></div>'
    th = 'style="padding:9px 14px;font-size:11px;color:#555;text-align:left;text-transform:uppercase;border-bottom:1px solid #e8e8e8;background:#f0f4fa;"'
    ts = 'width="100%" cellpadding="0" cellspacing="0" style="border:1px solid #e8e8e8;border-radius:8px;overflow:hidden;border-collapse:collapse;"'
...
<table {ts}>
<tr><td colspan="3" style="padding:12px 14px;background:#f7f9fc;border-bottom:1px solid #e8e8e8;"><strong style="font-size:15px;">AD - Andre Dailerian</strong><span style="color:#888;font-size:12px;margin-left:8px;">Grade 10 - Chatham HS - GPA 2.97</span></td></tr>
<tr><th {th} width="35%">Subject</th><th {th} width="15%" style="text-align:center;">Grade</th><th {th}>Due This Week</th></tr>
{a_rows}</table><br>
<table {ts}>
<tr><td colspan="3" style="padding:12px 14px;background:#f7f9fc;border-bottom:1px solid #e8e8e8;"><strong style="font-size:15px;">AR - Arina Dailerian</strong><span style="color:#888;font-size:12px;margin-left:8px;">Grade 7 - Chatham MS - GPA 3.79</span></td></tr>
<tr><th {th} width="35%">Subject</th><th {th} width="15%" style="text-align:center;">Grade</th><th {th}>Due This Week</th></tr>
{ar_rows}</table>
</td></tr><tr><td style="padding:16px 0;text-align:center;"><span style="font-size:11px;color:#aaa;">Dailerian School Tracker - Sent daily at 4:00 PM</span></td></tr>
...
def send_email():
    ...
    msg["Subject"] = subject
    msg["From"] = f"Dailerian School Tracker <{GMAIL_SENDER}>"
